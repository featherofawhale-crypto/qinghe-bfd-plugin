...
function UIBridge.get_params_uimanager(version_compat, timeline_list)
    ...
    local chk_render_nested = false  -- 复合/Fusion片段精查：默认关闭
    ...
    local win = disp:AddWindow({
        ...
        ui:VGroup{
            ...
            ui:Label{ ID = "AnalysisModeLabel", Text = "分析方式",
                      Font = ui:Font{ PixelSize = 11, Bold = true } },
            ui:CheckBox{ ID = "ChkMerge", Text = "成片模式（推荐，合并顶层可见素材分析）", Checked = config.MERGE_MODE.ENABLED },
            ui:CheckBox{ ID = "ChkRenderNested", Text = "  复合/Fusion片段精查（默认关闭 · 临时渲染顶层可见小段）", Checked = false },
            ui:CheckBox{ ID = "ChkComplexMode", Text = "  复杂工程模式（需填出入点 · 整段渲染后分析）", Checked = false },
    ...
    win.On.ChkRenderNested.Clicked = function(ev) chk_render_nested = ev.Checked ~= nil and ev.Checked or not chk_render_nested end
    ...
    win.On.StartBtn.Clicked = function(ev)
        ...
        params_result = {
            ...
            render_nested_segments = chk_render_nested,
...
function UIBridge.get_params_script()
    return {
        ...
        marker_types = {
            ...
            black_border = config.DEFAULT_MARKER_TYPES.black_border,
        },
        clear_old        = true,
        detect_duplicate = true,
        mark_hidden_clips = false,
