...
config.PLUGIN_VERSION = "2.0.1-beta.23"
...
config.MARKER_COLORS = {
    ...
    INFO    = "Cyan",     -- 信息性标记
    -- 透明度/合成检测
    OPACITY_HIDDEN   = "Lemon",    -- 隐藏素材（Resolve支持色，与Red区分）
    OPACITY_LOW      = "Mint",     -- 低透明度（Resolve支持色）
    OPACITY_PARTIAL  = "Cocoa",    -- 部分透明（Resolve支持色）
    ...
    BLACK_BORDER      = "Cream",   -- 画面边缘黑边（Resolve支持色，与远距重复区分）
}
...
config.DEFAULT_MARKER_TYPES = {
    ...
    black_border = false, -- 黑边检测 - 默认关闭
}

-- ============================================================
-- 画面边缘黑边检测参数
-- ============================================================
config.BLACK_BORDER_DETECTION = {
    ENABLED = false,
    MIN_BORDER_PX = 3,
    LIMIT = 0.02,
    SAMPLE_FPS = 2,
...
config.DUPLICATE = {
    ...
    DISTANT_SHORT_OVERLAP_FORCE_SEC = 5, -- 短源区间重复即使远距也标记，避免误复制漏报
...
config.OPACITY_DETECTION = {
    ...
    -- 只有 Resolve 明确返回非0合成模式时才打 [BFD-CMP]，读不到(nil)不能当成非标准合成。
    MARK_COMPOSITE_NONORMAL = true,
}
