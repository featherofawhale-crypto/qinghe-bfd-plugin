...
function Analyzer.get_marker_color(classification)
    local map = {
        ...
        black_border = config.MARKER_COLORS.BLACK_BORDER,
    }
...
function Analyzer.get_marker_name(classification)
    local map = {
        ...
        black_border = config.MARKER_NAMES.BLACK_BORDER,
    }
...
-- ============================================================
function Analyzer.compute_gap_ranges(coverage_table, timeline_fps, timeline_start_frame, timeline_end_frame)
    local gaps = {}
    if not coverage_table or #coverage_table == 0 then
        if timeline_start_frame and timeline_end_frame and timeline_end_frame > timeline_start_frame then
            table.insert(gaps, {
                start_frame = timeline_start_frame,
                end_frame = timeline_end_frame,
                duration_sec = (timeline_end_frame - timeline_start_frame) / timeline_fps,
            })
        end
        return gaps
    end
    local first_frame = timeline_start_frame or 0
    local last_frame = timeline_end_frame

    local clipped = {}
    for _, range in ipairs(coverage_table) do
        local start_frame = range.start_frame
        local end_frame = range.end_frame
        if start_frame and end_frame then
            if start_frame < first_frame then start_frame = first_frame end
            if last_frame and end_frame > last_frame then end_frame = last_frame end
            if end_frame > first_frame and (not last_frame or start_frame < last_frame) and end_frame > start_frame then
    ...
    if #clipped == 0 then
        if last_frame and last_frame > first_frame then
            table.insert(gaps, {
                start_frame = first_frame,
                end_frame = last_frame,
                duration_sec = (last_frame - first_frame) / timeline_fps,
            })
        end
        return gaps
    end
    table.sort(clipped, function(a, b) return a.start_frame < b.start_frame end)

    -- 检查开头是否有空白（第一个片段之前的空位）
    if clipped[1].start_frame > first_frame then
        table.insert(gaps, {
            start_frame = first_frame,
            end_frame = clipped[1].start_frame,
            duration_sec = (clipped[1].start_frame - first_frame) / timeline_fps,
        })
    end

    -- 检查片段之间的空白
    local merged = { clipped[1] }
    for i = 2, #clipped do
        local prev = merged[#merged]
        local cur = clipped[i]
        if cur.start_frame <= prev.end_frame then
            if cur.end_frame > prev.end_frame then prev.end_frame = cur.end_frame end
        else
            table.insert(merged, cur)
        end
    end
    for i = 1, #merged - 1 do
        local gap_start = merged[i].end_frame
        local gap_end = merged[i + 1].start_frame
    ...
    if last_frame and merged[#merged].end_frame < last_frame then
        table.insert(gaps, {
            start_frame = merged[#merged].end_frame,
            end_frame = last_frame,
            duration_sec = (last_frame - merged[#merged].end_frame) / timeline_fps,
        })
    end
...
function Analyzer.generate_note(segment, clip_info, timeline_fps, params, duration_frames)
    ...
    if segment.is_mixed_cut then
        ...
        if segment.final_visible_fragment then
            table.insert(parts, "位置: 源内镜头段与最终可见区间相交后只露出极短帧数")
        elseif segment.edge_visible then
        ...
        table.insert(parts, "判定: 以最终时间线实际露出的画面帧数为准；源内切点只用于拆分混剪成片镜头段。")
...
function Analyzer.analyze_results(ffmpeg_results, timeline_fps, params, clips)
    ...
    for _, result in ipairs(ffmpeg_results) do
        ...
        for _, segment in ipairs(result.segments) do
            ...
            -- 先按时长分类；少数上游补充记录（如普通成片模式的时间线空段）
            -- 已经知道真实类型时，可强制指定，避免被时长规则误判为夹帧/可疑黑帧。
            local classification = segment.force_classification or
                Analyzer.classify_segment(segment.duration, params, timeline_fps)
            ...
            if coverage_table and classification ~= "ignore" and classification ~= "black_border" then
                local gap_tolerance = segment.strict_gap_check and 0 or nil
                if Analyzer.is_in_gap(frames.start_frame, coverage_table, gap_tolerance) then
                    classification = "gap"
                end
            end
            ...
            local record = {
                ...
                marker_color = segment.force_marker_color or Analyzer.get_marker_color(classification),
                marker_name = segment.force_marker_name or
                    (segment.is_mixed_cut and "[BFD-MIX] 混剪源内夹帧" or Analyzer.get_marker_name(classification)),
            ...
            elseif classification == "black_border" then
                table.insert(results.black_borders, record)
                results.summary.black_border_count = results.summary.black_border_count + 1
            else
    ...
    if coverage_table and config.GAP_DETECTION.ENABLED then
        local gap_scan_start = params.in_point or params.start_offset
        local gap_scan_end = params.out_point
        local gap_ranges = Analyzer.compute_gap_ranges(coverage_table, timeline_fps, gap_scan_start, gap_scan_end)
        ...
        if params.in_point and params.out_point then
            ignore_above_sec = math.huge
        end
...
function Analyzer.get_filtered_marker_records(analyzed_results, enabled_types)
    ...
    if enabled_types.black_border then
        for _, r in ipairs(analyzed_results.black_borders or {}) do table.insert(records, r) end
    end
