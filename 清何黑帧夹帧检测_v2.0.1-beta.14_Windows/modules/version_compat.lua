...
local function get_resolve_app()
    ...
    if is_windows then
        ...
        compat_log("Skipping global Resolve() on macOS/Linux to avoid external fuscript hang")
    end
...
function VersionCompat:get_clip_transform(item)
    local values = {
        Pan = 0, Tilt = 0, ZoomX = 1, ZoomY = 1,
        RotationAngle = 0, AnchorPointX = 0, AnchorPointY = 0,
        Pitch = 0, Yaw = 0,
    }
    if not item then return values end
    for key, default in pairs(values) do
        pcall(function()
            local v = tonumber(item:GetProperty(key))
...
function VersionCompat:get_clip_crop(item)
    local values = {
        CropLeft = 0, CropRight = 0, CropTop = 0, CropBottom = 0, CropSoftness = 0,
    }
    if not item then return values end
    for key, default in pairs(values) do
        pcall(function()
