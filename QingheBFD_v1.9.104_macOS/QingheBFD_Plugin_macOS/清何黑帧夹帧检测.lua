...
function Main()
    ...
    local timeline_start_frame = compat:get_timeline_start_offset(timeline, timeline_fps)
    local timeline_start_tc = "00:00:00:00"
    pcall(function()
        if timeline.GetStartTimecode then
            timeline_start_tc = timeline:GetStartTimecode() or timeline_start_tc
        end
    end)
    if timeline_start_tc == "" or timeline_start_tc == "00:00:00:00" then
        pcall(function()
            timeline_start_tc = timeline:GetSetting("timelineStartTimecode") or timeline_start_tc
        end)
    end
    local timeline_start_display_frame = parse_tc_to_frames(timeline_start_tc, timeline_fps) or 0
    local function display_tc_to_timeline_frame(tc_str)
        local display_frame = parse_tc_to_frames(tc_str, timeline_fps)
        if display_frame == nil then return nil end
        return timeline_start_frame + math.max(0, display_frame - timeline_start_display_frame)
    end
    ...
    if params.manual_io_in and params.manual_io_in ~= "" then
        io_in = display_tc_to_timeline_frame(params.manual_io_in)
        dlog("阶段3.5: 手动入点=" .. params.manual_io_in .. " → " .. tostring(io_in) .. "帧")
    end
    if params.manual_io_out and params.manual_io_out ~= "" then
        io_out = display_tc_to_timeline_frame(params.manual_io_out)
    ...
    for _, item in ipairs(all_items) do
        ...
        if file_path and file_path ~= "" and media_type then
            ...
            pcall(function() end_frame = item:GetEnd() end)

            local left_offset = 0
            local left_offset_ok = false
            local source_dur = 0
            local ok_left, left_val = pcall(function() return item:GetLeftOffset() end)
            if ok_left and left_val ~= nil then
                left_offset = tonumber(left_val) or 0
                left_offset_ok = true
            end
            pcall(function() source_dur = item:GetDuration() end)
            if (not end_frame or end_frame <= start_frame) and source_dur > 0 then
                end_frame = start_frame + source_dur
            end
            if source_dur <= 0 and end_frame > start_frame then
                source_dur = end_frame - start_frame
            end
            ...
            table.insert(clips, {
                ...
                source_range_reliable = left_offset_ok and source_dur > 0,
    ...
    if has_io_range then
        ...
        for _, clip in ipairs(clips) do
            local clip_end = clip.timeline_end_frame or (clip.timeline_start_frame + clip.source_duration_frames)
            -- 判断片段是否与入出点范围有重叠
    ...
    if params.complex_mode then
        ...
        if not has_io_range then
            ...
            local render_ok, render_err = pcall(function()
                ...
                -- 复杂模式放宽pic_th（默认0.95太严，渲染视频有压缩噪点，0.30更灵敏）
                local pic_th = 0.30
                ...
                local scene_cmd = string.format(
                    '%s -i "%s" -vf "select=\'gt(scene\\,0.15)\',showinfo" -an -f null - 2>&1',
                    ffmpeg.ffmpeg_path, temp_path
                )
                ...
                if sf then
                    ...
                    for line in sf:lines() do
                        ...
                        local t = line:match("pts_time:(%d+%.?%d*)")
                        if t then table.insert(scene_times, tonumber(t)) end
                    end
                    sf:close()
                    ...
                    dlog(string.format("复杂模式：夹帧检测完成, 发现 %d 个极短场景(≤%d帧)", flash_count, stuck_f))
    ...
    if params.marker_types and params.marker_types.opacity then
        ...
        for _, clip in ipairs(clips) do
            ...
            elseif clip.composite_mode ~= 0 then
                ...
                local mode_str = mode_names[clip.composite_mode] or ("模式" .. clip.composite_mode)
    ...
    if params.merge_mode then
        ...
        -- 按时间线位置排序，确保concat顺序正确
        local sorted_clips = {}
        for _, c in ipairs(ffmpeg_clips) do table.insert(sorted_clips, c) end
        table.sort(sorted_clips, function(a, b)
        ...
        for _, clip in ipairs(sorted_clips) do
            local lo = clip.left_offset or 0
            local dur = clip.source_duration_frames or 0
            if dur > 0 and clip.file_path then
                local start_sec = lo / timeline_fps
                local dur_sec = dur / timeline_fps
                table.insert(segment_list, {
                    file_path = clip.file_path,
                    start_sec = start_sec,
                    duration_sec = dur_sec,
                })
                table.insert(seg_clips, clip)
                cum_total = cum_total + dur_sec
                table.insert(cum_end, cum_total)
            end
        end

        dlog("阶段6(成片): " .. #segment_list .. " 个有效片段, 总时长=" .. string.format("%.1f", cum_total) .. "秒")
            ...
            for _, seg in ipairs(all_segs) do
                ...
                for i = 1, #seg_clips do
                    ...
                    if seg_cum_start < b_end and cum_end[i] > b_start then
                        ...
                        local lo = clip.left_offset or 0
                        local src_start = lo / timeline_fps + (overlap_start - seg_cum_start)
                        local src_end = lo / timeline_fps + (overlap_end - seg_cum_start)
    ...
    local start_offset = timeline_start_frame
    ...
    if params.detect_duplicate ~= false and config.DUPLICATE.ENABLED then
        ...
        -- 8b: 基于帧指纹的内容重复检测
        local content_dup_enabled = (params.detect_content_dup == true)
            and (params.marker_types == nil or params.marker_types.content_dup == true)
            and config.DUPLICATE.CONTENT_DETECT_ENABLED
        dlog(string.format("阶段8b入口: detect_content_dup=%s marker_content_dup=%s CONTENT_DETECT_ENABLED=%s complex_render=%s",
            tostring(params.detect_content_dup),
            tostring(params.marker_types and params.marker_types.content_dup),
            tostring(config.DUPLICATE.CONTENT_DETECT_ENABLED),
            tostring(complex_render_done)))
        if content_dup_enabled then
