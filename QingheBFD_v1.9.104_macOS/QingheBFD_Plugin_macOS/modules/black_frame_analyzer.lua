...
function Analyzer.build_coverage_table(clips, timeline_fps)
    ...
    for _, clip in ipairs(clips) do
        ...
        local duration_frames = end_f - start_f
        if duration_frames <= 0 and clip.source_duration_frames and clip.source_duration_frames > 0 then
            duration_frames = clip.source_duration_frames
        end
        if duration_frames <= 0 and clip.item then
            pcall(function()
                duration_frames = clip.item:GetDuration()
            end)
        end

        end_f = start_f + math.max(0, duration_frames)
