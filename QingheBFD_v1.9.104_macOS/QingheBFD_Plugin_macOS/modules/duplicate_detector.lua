...
local function clip_timeline_duration(clip)
    local start_f = tonumber(clip.timeline_start_frame or clip.start_frame or 0) or 0
    local end_f = tonumber(clip.timeline_end_frame or clip.end_frame or 0) or 0
    if end_f > start_f then
        return end_f - start_f
    end
    return tonumber(clip.source_duration_frames or 0) or 0
end

local function clip_timeline_end(clip)
    local start_f = tonumber(clip.timeline_start_frame or clip.start_frame or 0) or 0
    local end_f = tonumber(clip.timeline_end_frame or clip.end_frame or 0) or 0
    if end_f > start_f then
        return end_f
    end
    return start_f + clip_timeline_duration(clip)
end

local function source_range_reliable(clip)
    return clip
        and clip.source_range_reliable == true
        and clip.left_offset_ok == true
        and clip.left_offset ~= nil
        and clip_timeline_duration(clip) > 0
end

local function source_overlap_frames(clip_a, clip_b)
    if not source_range_reliable(clip_a) or not source_range_reliable(clip_b) then
        return nil
    end
    local src_a_start = tonumber(clip_a.left_offset or 0) or 0
    local src_b_start = tonumber(clip_b.left_offset or 0) or 0
    local src_a_end = src_a_start + clip_timeline_duration(clip_a)
    local src_b_end = src_b_start + clip_timeline_duration(clip_b)
    return math.min(src_a_end, src_b_end) - math.max(src_a_start, src_b_start)
end
...
function DuplicateDetector.detect(clips, timeline_fps, params)
    ...
    local function build_gap_coverage(all_clips)
        ...
        for _, c in ipairs(all_clips) do
            ...
            local d = clip_timeline_duration(c)
    ...
    local function analyze_group(group, match_type, threshold_frames, result_list, count_field)
        ...
        for i = 1, #group - 1 do
            for j = i + 1, #group do
                ...
                local a_end = clip_timeline_end(clip_a)

                local b_start = clip_b.timeline_start_frame
                local b_end = clip_timeline_end(clip_b)
                ...
                if match_type == "same_file" then
                    local overlap_frames = source_overlap_frames(clip_a, clip_b)

                    -- 读不到源入点时不能证明是同一段素材；跳过，避免长素材正常切段误报
                    if overlap_frames == nil or overlap_frames <= 0 then
                        goto skip_entry
                    end
                elseif match_type == "same_name_duration" then
                    local path_a = normalize_path_for_compare(clip_a.file_path)
                    local path_b = normalize_path_for_compare(clip_b.file_path)
                    if path_a and path_b and path_a == path_b then
                        goto skip_entry
                    end
                end
        ...
        if #group >= 2 then
            ...
            for i = 1, #group - 1 do
                for j = i + 1, #group do
                    ...
                    -- 有长空档且总距离大 → 故意复用，跳过
                    if has_long_empty_gap(a_end, b_start) and dist_sec > min_total_sec then
...
function DuplicateDetector.to_marker_records(dup_results, timeline_fps)
    ...
    local function add_dup_records(dup, color, name_a, name_b)
        ...
        local end_a = clip_timeline_end(dup.clip_a)
        local end_b = clip_timeline_end(dup.clip_b)
...
function DuplicateDetector._find_dup_ranges(fingerprints, min_gap_sec, fps)
    ...
    -- 计算采样间隔（用于后置重叠判断）
    local sample_interval_frames = 3
        ...
        if not used[i] then
            ...
            for j = i + 1, #pairs_found do
                if not used[j] then
                    ...
                    if np.idx1 - seg.end1_idx_fp <= 2 and math.abs(np.idx2 - seg.end2_idx_fp) <= 2 then
                        seg.end1 = np.sec1
            ...
            if seg.count >= 2 then  -- 至少2个连续匹配点（降低阈值，检测更短重复）
...
function DuplicateDetector.detect_content(clips, timeline_fps, params)
    ...
    do
        ...
        for _, clip in ipairs(clips) do
            ...
            if file_path then
                local fingerprints = DuplicateDetector._extract_fingerprints(
                    file_path, interval_frames, thumb_size, timeline_fps,
                    clip.left_offset and clip.left_offset > 0 and (clip.left_offset / timeline_fps) or nil,
                    clip.source_duration_frames and clip.source_duration_frames > 0 and (clip.source_duration_frames / timeline_fps) or nil
                )
                if #fingerprints > 0 then
                ...
                for a = 1, #group do
                    for b = a + 1, #group do
                        ...
                        if fp_i.file_path ~= fp_j.file_path then
                            local dist = hamming_distance_hex(fp_i.edge_hash, fp_j.edge_hash)
                            if dist <= MAX_HAMMING then
                                table.insert(matched_pairs, {i = i, j = j, dist = dist})
                            end
                        end
                    end
                end
...
function DuplicateDetector.detect_content_render(render_path, io_in_frame, timeline_fps, params)
    ...
    local results = {
        summary = { files_scanned = 0, total_fingerprints = 0, files_with_duplicates = 0 },
        results = {},
    }
    ...
    -- 用虚拟clip承载渲染文件结果
    local dummy_clip = {
        timeline_start_frame = io_offset,
...
function DuplicateDetector.content_to_marker_records(content_results, timeline_fps)
    ...
    for _, file_result in ipairs(content_results.results) do
        ...
        local clip_start = clip.timeline_start_frame

        for _, range in ipairs(file_result.ranges) do
            -- 映射到时间线帧
            local tl_frame1_start = clip_start + range.frame1_start
            local tl_frame1_end = clip_start + range.frame1_end
            local tl_frame2_start = clip_start + range.frame2_start
            local tl_frame2_end = clip_start + range.frame2_end
            ...
            local note = string.format(
                "内容重复!\n段1: 帧%d-%d (%.1fs)\n段2: 帧%d-%d (%.1fs)\n间距: %.1fs\n来源: %s",
                range.frame1_start, range.frame1_end, dur_sec1,
                range.frame2_start, range.frame2_end, dur_sec2,
                gap_sec,
            ...
            table.insert(records, {
                ...
                duration_frames = range.frame1_end - range.frame1_start,
            })
            ...
            table.insert(records, {
                ...
                duration_frames = range.frame2_end - range.frame2_start,
            })
        end
    end
...
function DuplicateDetector.generate_content_summary(content_results)
    local s = content_results.summary
    local lines = {}
    table.insert(lines, string.rep("-", 55))
    ...
    table.insert(lines, string.format("  🟣 发现内容重复: %d 个文件, %d 处重复区间",
        s.files_with_duplicates, s.total_dup_ranges))
    return table.concat(lines, "\n")
end
