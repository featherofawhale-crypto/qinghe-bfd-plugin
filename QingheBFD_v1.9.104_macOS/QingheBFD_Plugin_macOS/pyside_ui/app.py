...
class MainWindow(QMainWindow):
    ...
    def _build_timeline_group(self) -> QGroupBox:
        ...
        self.full_timeline_btn.setMinimumWidth(104)
        self.full_timeline_btn.setMaximumWidth(124)
        ...
        self.io_in.setMinimumWidth(120)
        ...
        self.io_out.setMinimumWidth(120)
        ...
        self.batch_timeline_list.setMinimumHeight(50)
        self.batch_timeline_list.setMaximumHeight(78)
        ...
        layout.addWidget(self.full_timeline_btn, 2, 1)
        layout.addWidget(self.chk_batch_timelines, 3, 0)
        layout.addWidget(self.batch_timeline_list, 3, 1, 1, 4)
    ...
    def _build_font_tab(self) -> QWidget:
        ...
        set_tip(self.font_copy_style_btn, "复制选中 Text+ 的外观参数，不复制文字内容。")
        self.font_copy_style_btn.clicked.connect(self.copy_selected_textplus_style)
        self.font_apply_style_btn = QPushButton("应用样式")
        set_tip(self.font_apply_style_btn, "把已复制样式应用到选中的一个或多个 Text+，保留每层原来的文字。支持 Shift/Cmd 多选。")
        ...
        set_tip(self.font_convert_srt_btn, "把当前时间线启用的 SRT 字幕尽量转换成 Text+ 标题层；Resolve 不支持时会明确提示。")
    ...
    def _build_audio_tab(self) -> QWidget:
        ...
        self.audio_apply_preset_btn = QPushButton("应用预设")
        self.audio_apply_preset_btn.setIcon(self.style().standardIcon(QStyle.SP_DialogApplyButton))
        set_tip(self.audio_apply_preset_btn, "显示达芬奇内置 Fairlight FX 参数卡：效果器名称、添加位置和建议数值；当前不会写入工程。")
        self.audio_apply_preset_btn.clicked.connect(self.apply_audio_preset)
        preset_top.addWidget(self.audio_preset_combo, 1)
        preset_top.addWidget(self.audio_apply_preset_btn)
    ...
    def audio_preset_card_text(self, preset: dict) -> str:
        ...
        lines = [
            ...
            "操作：Fairlight 页 > Mixer > 对应轨道/片段 > Effects/FX 插槽；EQ/Dynamics 可直接用通道条。",
            "",
            "强度三档：",
        ]
    ...
    def apply_audio_preset(self) -> None:
        ...
        name = str((preset or {}).get("name", "音频预设")) if isinstance(preset, dict) else "音频预设"
        if isinstance(preset, dict):
            self.audio_preset_detail.setPlainText(self.audio_preset_card_text(preset))
        self.audio_summary.setText(
            f"{name}：已显示达芬奇内置 Fairlight FX 参数卡。当前 API 不会一键写入工程。"
        )
    ...
    def prompt_complex_mode_for_risky_timelines(self, jobs: list[dict]) -> bool:
        ...
        for job in jobs:
            ...
            if not any(token in timeline_name.lower() for token in risky_tokens):
                continue
            prompt = (
                f"时间线“{timeline_name}”命名疑似混剪/成片。\n\n"
                "普通模式只做结构和源素材检测，不会渲染最终画面；如果要检查调色、OFX、Fusion、叠加后的最终画面，请启用复杂模式。\n\n"
                "是否现在切换到复杂模式？"
            )
            answer = QMessageBox.question(
                self,
    ...
    def apply_font_to_selected_layer(self) -> None:
        ...
        if not record.get("supported"):
            ...
            self._log(self.font_status.text())
            return
        result = self.bridge.replace_font_item(record, font_name, self.font_candidates(font_name))
        self.font_status.setText(str(result.get("message", "")))
        self._log(self.font_status.text())
        if result.get("ok"):
    ...
    def convert_srt_to_textplus(self) -> None:
        ...
        QApplication.processEvents()
        result = self.bridge.convert_srt_to_textplus(int(selected.get("index", 1)))
