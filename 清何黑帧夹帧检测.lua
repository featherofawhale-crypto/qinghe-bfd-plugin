...
local function remove_complex_render_cache(cache_dir, sep)
    ...
    os.remove(cache_dir .. sep .. "bfd_temp_render.mp4.qinghe_bfd_meta.lua")
    os.remove(cache_dir .. sep .. "bfd_temp_render.mov.qinghe_bfd_meta.lua")
end

local function shell_quote_path(path, sep)
...
local function read_file_prefix(path, max_bytes)
    ...
    local content = f:read(max_bytes or 8192) or ""
    f:close()
    return content
end

local function list_progress_files(state_dir, sep)
    local cmd
    if sep == "\\" then
        cmd = 'dir /b /a:-d "' .. state_dir .. sep .. 'progress*.json" 2>nul'
    else
        cmd = "find " .. shell_quote_path(state_dir, sep) .. " -maxdepth 1 -type f -name " .. shell_quote_path("progress*.json", sep) .. " -print 2>/dev/null"
    end
    ...
    for line in handle:lines() do
        if line and line ~= "" then
            local path = line
            if sep == "\\" and not line:match("^%a:[/\\]") and not line:match("^\\\\") then
                path = state_dir .. sep .. line
            end
...
local function has_recent_running_progress(state_dir, sep)
    local now = os.time()
    for _, path in ipairs(list_progress_files(state_dir, sep)) do
        local content = read_file_prefix(path, 8192)
        if content:find('"state"%s*:%s*"running"') then
            local updated_at = tonumber(content:match('"updated_at"%s*:%s*(%d+)') or "")
            if not updated_at or (now - updated_at) < (12 * 60 * 60) then
                return true, path
            end
        end
    end
...
local function cleanup_stale_complex_render_cache_on_startup()
    local sep = package.config:sub(1, 1)
    local home = os.getenv("USERPROFILE") or os.getenv("HOME") or "."
    local state_dir = home .. sep .. ".qinghe_bfd"
    local cache_dir = state_dir .. sep .. "render_cache"
    local has_temp =
        file_exists(cache_dir .. sep .. "bfd_temp_render.mp4") or
        file_exists(cache_dir .. sep .. "bfd_temp_render.mov") or
        file_exists(cache_dir .. sep .. "bfd_temp_render.mp4.qinghe_bfd_meta.lua") or
        file_exists(cache_dir .. sep .. "bfd_temp_render.mov.qinghe_bfd_meta.lua")
    if not has_temp then return end
    local active, progress_file = has_recent_running_progress(state_dir, sep)
    if active then
        dlog("启动缓存清理跳过：检测仍在运行 progress=" .. tostring(progress_file))
        return
    end
    remove_complex_render_cache(cache_dir, sep)
    dlog("启动缓存清理：已删除上次残留复杂模式临时渲染")
end
...
dlog("所有模块加载完成")
cleanup_stale_complex_render_cache_on_startup()
...
function Main()
    ...
    local function timeline_frame_has_any_clip(frame)
        frame = tonumber(frame)
        if not frame then return false end
        frame = math.floor(frame + 0.5)
        for _, clip in ipairs(clips or {}) do
            local s = tonumber(clip.timeline_start_frame)
            local dur = tonumber(clip.source_duration_frames or 0) or 0
            if (not dur or dur <= 0) and clip.item then
                pcall(function() dur = tonumber(clip.item:GetDuration() or 0) or 0 end)
            end
            if s and dur and dur > 0 then
                local e = s + dur
                if frame >= s and frame < e then return true end
            end
        end
        return false
    end
    ...
    local function append_rendered_black_border_results(ffmpeg, rendered_path, timeline_start_frame, source_clip, opts)
        ...
        opts = opts or {}
        local dark_stats_for_border = opts.dark_stats or {}
        local function is_full_black_or_timeline_gap(local_frame)
            local f = math.max(0, math.floor(tonumber(local_frame or 0) or 0))
            if (tonumber(dark_stats_for_border[f] or 0) or 0) >= 95 then return true end
            local timeline_frame = (tonumber(timeline_start_frame or 0) or 0) + f
            if not timeline_frame_has_any_clip(timeline_frame) then return true end
            return false
        end
        ...
        if edge_hits and #edge_hits > 0 then
            ...
            if is_full_black_or_timeline_gap(hit_frame) then
                dlog("最终画面黑边：跳过整黑帧/时间线空白帧 frame=" .. tostring(hit_frame))
                return 0
            end
            local sides = {}
            local side_seen = {}
        ...
        if is_full_black_or_timeline_gap(0) then
            dlog("最终画面黑边：跳过从整黑帧/时间线空白起标的内容框结果")
            return 0
        end
            ...
            local render_ok, render_err = pcall(function()
                ...
                local temp_path = cache_dir .. sep .. render_name .. ".mp4"
	                ...
	                if imported_path ~= "" then
	                    ...
	                    if file_exists(complex_render_meta_path(imported_path)) then
	                        ...
	                        print("[BFD] 使用用户指定外部成片，跳过本次整段渲染。")
	                        dlog("复杂模式：外部成片无清何缓存匹配文件，按当前IO/FPS映射直接分析")
	                    end
	                else
                    if params.keep_complex_cache ~= true then
                ...
                local dummy_clip = {
	                ...
	                if analyzed and #analyzed > 0 then
	                    ...
	                    for _, seg in ipairs(analyzed) do
	                        ...
	                        local search_start = math.max(0, s_frame - 2)
	                        local search_end = math.max(search_start, e_frame + 2)
	                        local dark_start, dark_end, pblack = nil, nil, 0
	                        for f = search_start, search_end do
	                            local p = tonumber(dark_stats[f] or 0) or 0
	                            if p >= 55 then
	                                if not dark_start then dark_start = f end
	                                dark_end = f + 1
	                                if p > pblack then pblack = p end
	                            elseif dark_start and f > s_frame then
	                                break
	                            end
	                        end
	                        if not dark_start then
	                            pblack = max_dark_pblack(s_frame, e_frame, 1)
	                        end
	                        if pblack >= 55 then
	                            local mapped_start = dark_start or s_frame
	                            local mapped_end = math.max(mapped_start + 1, dark_end or e_frame)
	                            table.insert(filtered_analyzed, seg)
	                            table.insert(ffmpeg_results, {
	                                clip = dummy_clip,
	                                segments = {{
	                                    start = mapped_start / timeline_fps,
	                                    end_ = mapped_end / timeline_fps,
	                                    duration = (mapped_end - mapped_start) / timeline_fps,
	                                    duration_frames = math.max(1, mapped_end - mapped_start),
	                                    dark_pblack = pblack,
	                                    timeline_frame = io_in + mapped_start,
	                                    timeline_end_frame = io_in + mapped_end,
	                                    strict_gap_check = true,
	                                }},
                ...
                if #scene_times > 0 then
                    ...
                    for _, st in ipairs(scene_times) do
	                    ...
	                    local boundary_threshold = tonumber(params.render_flash_boundary_scene_score or 0.08) or 0.08
	                    local flash_count = 0
	                    local skipped_dark_flash = 0
	                    local skipped_no_hash = 0
	                    local skipped_not_returning = 0
	                    local flash_candidates = {}
	                    local flash_candidate_keys = {}
	                    ...
	                    local function count_dense_scene_points(start_frame, end_frame)
	                        local dense_count = 0
	                    ...
	                    local function queue_flash_candidate(start_frame, end_frame, gap, start_score, end_score, source_kind)
	                        start_frame = math.max(0, math.floor(tonumber(start_frame or 0) or 0))
	                        end_frame = math.max(start_frame + 1, math.floor(tonumber(end_frame or start_frame + 1) or (start_frame + 1)))
	                        local short_len = end_frame - start_frame
	                        if short_len <= 0 or short_len > stuck_f then return false end
	                        local key = tostring(start_frame) .. ":" .. tostring(end_frame)
	                        if flash_candidate_keys[key] then return false end
	                        flash_candidate_keys[key] = true

	                        start_score = start_score or scene_scores[tostring(start_frame)] or 0
	                        end_score = end_score or scene_scores[tostring(end_frame)] or 0
	                        local scene_score = math.max(start_score, end_score)
	                        ...
	                        for forward = 0, 2 do add_frame(post_frames, end_frame + forward) end
	                        if #pre_frames > 0 and #mid_frames > 0 and #post_frames > 0 then
	                            table.insert(flash_candidates, {
	                                scene_start_sec = start_frame / timeline_fps,
	                                scene_end_sec = end_frame / timeline_fps,
	                                start_frame = start_frame,
	                                end_frame = end_frame,
	                                gap = gap or (short_len / timeline_fps),
	                                scene_score = scene_score,
	                                scene_score_start = start_score,
	                                scene_score_end = end_score,
	                                pblack = pblack,
	                                dense_count = count_dense_scene_points(start_frame, end_frame),
	                                pre_frames = pre_frames,
	                                mid_frames = mid_frames,
	                                post_frames = post_frames,
	                                source_kind = source_kind or "adjacent_scene",
	                            })
	                            return true
	                        end
	                    ...
	                    for i = 1, #scene_times - 1 do
	                        ...
	                        if gap > 0 and gap <= min_gap then
	                            ...
	                            local start_score = scene_scores[tostring(start_frame)] or 0
	                            local end_score = scene_scores[tostring(end_frame)] or 0
	                            queue_flash_candidate(start_frame, end_frame, gap, start_score, end_score, "adjacent_scene")
	                        end
	                    end
	                    for _, st in ipairs(scene_times) do
	                        local start_frame = math.floor(st * timeline_fps + 0.5)
	                        local start_score = scene_scores[tostring(start_frame)] or 0
	                        if start_score >= boundary_threshold then
	                            for short_len = 1, stuck_f do
	                                local end_frame = start_frame + short_len
	                                queue_flash_candidate(start_frame, end_frame, short_len / timeline_fps,
	                                    start_score, scene_scores[tostring(end_frame)] or 0, "single_boundary")
	                            end
	                        end
	                    end
	                    local frame_hashes = {}
                    ...
                    for _, cand in ipairs(flash_candidates) do
                        ...
                        local short_len = math.max(1, cand.end_frame - cand.start_frame)
                        local both_boundaries_strong =
                            (cand.scene_score_start or 0) >= boundary_threshold and
                            (cand.scene_score_end or 0) >= boundary_threshold
                        local has_boundary_support = both_boundaries_strong or
                            (cand.source_kind == "single_boundary" and (cand.scene_score_start or 0) >= boundary_threshold)
                        local middle_detaches_both_sides =
                            pre_mid_dist >= diff_threshold and post_mid_dist >= diff_threshold
                        local abc_short_flash =
                            (not returning_same_shot) and short_len <= stuck_f and
                            has_boundary_support and middle_detaches_both_sides
                        if pre_post_dist >= 9999 or mid_diff >= 9999 then
                            skipped_no_hash = skipped_no_hash + 1
                        elseif (returning_same_shot and middle_is_different) or abc_short_flash then
                            local flash_note
                            if returning_same_shot and middle_is_different then
                                flash_note = string.format(
                                    "最终渲染画面短镜头夹帧\n持续: %d帧\n切点: %d → %d\nscene score: %.3f\n成片结构验证: 前后相似距离=%d，中段差异距离=%d\n判定: 短段前后回到同一画面结构，中间闪入不同画面；不依赖时间线 Fusion/复合片段信息。",
                                    short_len, cand.start_frame, cand.end_frame,
                                    cand.scene_score, pre_post_dist, mid_diff
                                )
                            else
                                flash_note = string.format(
                                    "最终渲染画面短镜头夹帧\n持续: %d帧\n切点: %d → %d\nscene score: %.3f / %.3f\n成片结构验证: 前后差异距离=%d，前-中距离=%d，后-中距离=%d\n判定: 前后已经切到不同画面，但中间短段同时脱离两侧画面结构；按最终画面实际只露出极短帧数标记。",
                                    short_len, cand.start_frame, cand.end_frame,
                                    cand.scene_score_start or 0, cand.scene_score_end or 0,
                                    pre_post_dist, pre_mid_dist, post_mid_dist
                                )
                            end
                            local flash_result = {
                                clip = dummy_clip,
                    ...
                    dlog(string.format("复杂模式：夹帧检测完成, 候选 %d 个, 验证通过 %d 个(≤%d帧), 合并近邻 %d 个, 跳过黑帧候选 %d 个, 跳过结构未通过 %d 个, 跳过无hash %d 个",
        ...
        for _, clip in ipairs(clips) do
            ...
            elseif clip.opacity <= opacity_config.HIDDEN_THRESHOLD then
                ...
                if opacity_config.CHECK_LOWER_TRACKS and has_lower_track_content(clip.track_index, ts, te) then
                    ...
                    color = config.MARKER_COLORS.OPACITY_HIDDEN or "Lemon"
            ...
            elseif clip.opacity < opacity_config.LOW_OPACITY_THRESHOLD then
                ...
                color = config.MARKER_COLORS.OPACITY_LOW or "Mint"
            ...
            elseif clip.opacity < opacity_config.PARTIAL_OPACITY_THRESHOLD then
                if params.mark_partial_opacity ~= false then
                    ...
                    color = config.MARKER_COLORS.OPACITY_PARTIAL or "Cocoa"
    ...
    if #opacity_results > 0 and marker_types.opacity ~= false then
        for _, r in ipairs(opacity_results) do
            ...
            if not r.marker_color and r.color then r.marker_color = r.color end
            table.insert(selected_records, r)
        end
    ...
    do
        ...
        local function is_direct_visible_short_record(r)
            local name = tostring(r and r.marker_name or "")
            return name:find("%[BFD%-MIX%].-真实画面短镜头夹帧") ~= nil
                or is_external_overlay_short_record(r)
                or (r and r.nested_short_scene == true)
                or (r and r.segment and r.segment.nested_short_scene == true)
        end
        local function window_key(ws, we)
            return tostring(ws) .. ":" .. tostring(we)
        end

        local direct_by_window = {}
        local direct_short_records = {}
        for _, r in ipairs(selected_records) do
            if is_direct_visible_short_record(r) then
                local ws, we = short_visual_window(r)
                if ws and we then
                    table.insert(direct_short_records, r)
                    local key = window_key(ws, we)
                    local current = direct_by_window[key]
                    local current_start = record_start_frame(current)
                    local r_start = record_start_frame(r)
                    if not current or not current_start or (r_start and r_start < current_start) then
                        direct_by_window[key] = r
                    end
                end
            end
        end

        local folded_source_evidence = 0
        local skipped_ambiguous_source_fold = 0
        local source_folded_records = {}
        for _, r in ipairs(selected_records) do
            if is_visible_fragment_mixed_record(r) then
                local ws, we = short_visual_window(r)
                local anchor = ws and we and direct_by_window[window_key(ws, we)] or nil
                local anchor_ws, anchor_we = ws, we
                if not anchor then
                    local f = record_start_frame(r)
                    local candidates = {}
                    for _, direct in ipairs(direct_short_records) do
                        local dws, dwe = short_visual_window(direct)
                        if dws and dwe and f and f >= dws and f < dwe
                            and ws and we and ws < dwe and we > dws
                        then
                            table.insert(candidates, { record = direct, ws = dws, we = dwe })
                        end
                    end
                    if #candidates == 1 then
                        anchor = candidates[1].record
                        anchor_ws = candidates[1].ws
                        anchor_we = candidates[1].we
                    elseif #candidates > 1 then
                        skipped_ambiguous_source_fold = skipped_ambiguous_source_fold + 1
                    end
                end
                if anchor and anchor ~= r then
                    folded_source_evidence = folded_source_evidence + 1
                    local original_frame = record_start_frame(r) or ws
                    local last_frame = math.max(anchor_ws, anchor_we - 1)
                    anchor.note = tostring(anchor.note or "") .. string.format(
                        "\n合并证据: 同一最终可见短窗口 %s-%s (%d帧) 内，源内混剪短镜头证据原本命中 %s；已保留最终画面第一帧标记，避免同一镜头段重复打点。",
                        Analyzer.frame_to_timecode(anchor_ws, timeline_fps),
                        Analyzer.frame_to_timecode(last_frame, timeline_fps),
                        anchor_we - anchor_ws,
                        Analyzer.frame_to_timecode(original_frame, timeline_fps)
                    )
                else
                    table.insert(source_folded_records, r)
                end
            else
                table.insert(source_folded_records, r)
            end
        end
        if folded_source_evidence > 0 then
            print(string.format("[BFD] 最终可见短段证据合并: 折叠 %d 条源内重复证据到第一帧标记", folded_source_evidence))
            dlog("阶段9: 同一最终可见短窗口源内证据折叠 folded_source_evidence="
                .. tostring(folded_source_evidence)
                .. " skipped_ambiguous=" .. tostring(skipped_ambiguous_source_fold))
            selected_records = source_folded_records
        end
    end
