...
- 素材归纳是 Resolve 项目管理的补充：扫描已用但误放在项目目录外的素材，在 Finder 定位、复制归位并按需自动重链接
- 可从源路径前端或末端保留指定目录层级，执行前直接预览每个素材的目标目录
- 归纳计划和复制在后台执行，显示实时进度与速度；删除源文件受复制验证和重链接结果双重保护
- 字体收集覆盖 Text+、SRT 字幕轨和普通 Text；系统未安装的字体会明确显示未匹配
...
- Windows：`QingheBFD_v2.0.1-beta.30_Windows_Setup.exe`
...
1. 下载并运行 `QingheBFD_v2.0.1-beta.30_Windows_Setup.exe`。
2. 按安装器提示完成安装。
...
插件需要通过 DaVinci Resolve 官方脚本接口读取当前项目、时间线、字幕、字体和媒体池。若没有开启脚本控制，可能会出现“API 无法连接”、扫描不到时间线、媒体池为空、字幕或字体功能无法读取等问题。

请在 DaVinci Resolve 中设置：

1. 打开 `DaVinci Resolve -> 偏好设置 -> 系统 -> 常规`。
2. 找到“外部脚本”设置。
3. 设置为“本地”或“网络”。
4. 保存设置并重启 DaVinci Resolve。
5. 打开具体项目和时间线后，再从 `工作区 -> 脚本 -> Edit` 启动插件。
