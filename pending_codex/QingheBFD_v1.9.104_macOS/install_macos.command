#!/bin/bash
# Qinghe BFD v2.0.1-beta.14 - macOS one-click installer.
...
pause_before_exit() {
    if [ "${QINGHE_AUTO_UPDATE:-0}" != "1" ]; then
        read -r -p "按回车退出..."
    fi
}
...
mkdir -p "$EDIT_DIR" "$COMP_DIR" "$MODULE_DIR"
...
if [ -f "$EDIT_DIR/$MAIN_SCRIPT" ] || [ -d "$MODULE_DIR" ]; then
    ...
    [ -f "$COMP_DIR/$MAIN_SCRIPT" ] && cp "$COMP_DIR/$MAIN_SCRIPT" "$BACKUP_DIR/Comp_$MAIN_SCRIPT"
    [ -f "$SCRIPT_BASE/Modules/$MAIN_SCRIPT" ] && cp "$SCRIPT_BASE/Modules/$MAIN_SCRIPT" "$BACKUP_DIR/ModulesRoot_$MAIN_SCRIPT"
    find "$MODULE_DIR" -maxdepth 1 -type f -name "*.lua" -exec cp {} "$BACKUP_DIR/" \; 2>/dev/null || true
fi

rm -f "$EDIT_DIR/$MAIN_SCRIPT" "$SCRIPT_BASE/$MAIN_SCRIPT" "$COMP_DIR/$MAIN_SCRIPT" "$SCRIPT_BASE/Modules/$MAIN_SCRIPT" 2>/dev/null || true
...
cp "$SOURCE_DIR/$MAIN_SCRIPT" "$COMP_DIR/"
...
if [ -d "$SOURCE_DIR/ffmpeg" ]; then
    ...
    xattr -dr com.apple.quarantine "$MODULE_DIR/ffmpeg" 2>/dev/null || true
    xattr -dr com.apple.quarantine "$SOURCE_DIR/ffmpeg" 2>/dev/null || true
fi

if [ -d "$SOURCE_DIR/pyside_ui" ]; then
    STYLE_LIBRARY_BACKUP=""
    if [ -f "$MODULE_DIR/pyside_ui/data/font_style_library.json" ]; then
        STYLE_LIBRARY_BACKUP="$(mktemp "${TMPDIR:-/tmp}/qinghe_font_style_library.XXXXXX.json")"
        cp "$MODULE_DIR/pyside_ui/data/font_style_library.json" "$STYLE_LIBRARY_BACKUP" 2>/dev/null || STYLE_LIBRARY_BACKUP=""
    fi
    rm -rf "$MODULE_DIR/pyside_ui"
    cp -a "$SOURCE_DIR/pyside_ui" "$MODULE_DIR/"
    if [ -n "$STYLE_LIBRARY_BACKUP" ] && [ -f "$STYLE_LIBRARY_BACKUP" ]; then
        mkdir -p "$MODULE_DIR/pyside_ui/data"
        cp "$STYLE_LIBRARY_BACKUP" "$MODULE_DIR/pyside_ui/data/font_style_library.json" 2>/dev/null || true
        rm -f "$STYLE_LIBRARY_BACKUP" 2>/dev/null || true
        echo "OK: 已保留本地 Text+ 样式库"
    fi
...
FAIL=0
for file in "$EDIT_DIR/$MAIN_SCRIPT" "$SCRIPT_BASE/$MAIN_SCRIPT" "$COMP_DIR/$MAIN_SCRIPT" "$MODULE_DIR/config.lua" "$MODULE_DIR/ffmpeg_runner.lua" "$MODULE_DIR/black_frame_analyzer.lua" "$MODULE_DIR/duplicate_detector.lua"; do
    if [ -f "$file" ]; then
        echo "OK: $file"
    else
        echo "缺失: $file"
