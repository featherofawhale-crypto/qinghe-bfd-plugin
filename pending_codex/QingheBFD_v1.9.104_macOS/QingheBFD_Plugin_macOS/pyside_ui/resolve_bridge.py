...
'''
    ...
    def convert_srt_to_textplus(self, timeline_index: int = 1, template_uid: str = "") -> dict[str, Any]:
        return self._font_action(int(timeline_index), "convert_srt_textplus", item={"caption_template_uid": template_uid})
    ...
    def _font_action(
        ...
        action = action if action in {"scan", "jump", "replace", "copy_style", "apply_style", "convert_srt_textplus", "list_caption_templates", "caption_conversion_info"} else "scan"
        candidates: list[str] = []
...
def media_item_type(media_item):
    ...
    value = ""
    if isinstance(props, dict):
        value = str(props.get("Type") or props.get("Clip Type") or props.get("Usage") or "")
    return value
...
if ACTION == "convert_srt_textplus":
    try:
        ...
        if template_item is None and os.path.exists(CAPTION_TEMPLATE_PATH):
            ...
            if template_item is None:
                template_warning = "内置 .drb 模板未被当前 Resolve 接受，已改用内置 Text+ 降级插入。"
        elif template_item is None:
            template_warning = "缺少内置 .drb 模板，已改用内置 Text+ 降级插入。"
        ...
        for entry in entries:
            if int(entry["start"]) in created_by_start:
                continue
            created_clip = None
            dur = max(1, int(entry["end"]) - int(entry["start"]))
...
''',
            timeout=300 if action == "convert_srt_textplus" else 45,
        )
        if not data:
...
def add_audio_clip_marker(item, track_index, reason, start_frame, end_frame):
    ...
    custom_data = custom_root + "-" + str(track_index) + "-" + str(start_frame)
    attempts = (
...
for track_index in range(1, track_count + 1):
    ...
    for item in items:
        ...
        source_is_mono = source_mapping_is_mono or media_mapping_is_mono or property_is_mono
...
if ACTION == "mark":
    if len(clips) == 0:
        if resolve_major and not timeline_audio_mapping_supported:
