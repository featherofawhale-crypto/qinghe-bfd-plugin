...
class PySideUiRegressionTests(unittest.TestCase):
    ...
    def test_version_is_test_build(self) -> None:
        self.assertEqual(self.pyside_app.APP_VERSION, "2.0.1-beta.2")
    ...
    def test_localized_chinese_font_names_resolve_to_system_families_first(self) -> None:
        ...
        self.assertIn("兰亭黑-简|||Extralight", lanting_candidates)
        ...
        if "云峰飞云体" in window.font_aliases:
            yunfeng_candidates = window.font_candidates("云峰飞云体 Regular")
            self.assertIn("YUNFENFEIYUNTI|||Regular", yunfeng_candidates)
            self.assertIn("YUNFENFEIYUNTI", yunfeng_candidates)
            self.assertTrue(
                any(candidate.startswith("云峰飞云体|||Regular|||") for candidate in yunfeng_candidates),
                "Resolve font candidates should carry the local font file path for FontManager.AddFont",
            )
    ...
    def test_postscript_font_names_map_back_to_textplus_family_and_style(self) -> None:
        ...
        self.assertIn("FZYOUHS_506L--GB1-0", candidates)

    def test_font_probe_records_success_and_failure_with_alias_keys(self) -> None:
        source = (PYSIDE_DIR / "app.py").read_text(encoding="utf-8")
        bridge_source = (PYSIDE_DIR / "resolve_bridge.py").read_text(encoding="utf-8")

        self.assertIn("def font_rule_source_keys", source)
        self.assertIn('"ok": ok', source)
        self.assertIn('"source_keys": source_keys[:12]', source)
        self.assertIn('"rejected": rejected[:16]', source)
        self.assertIn('"accepted_candidate": scrub_candidate(str(result.get("accepted_candidate") or ""))', source)
        self.assertIn('"registered_font_file": registered_font_file', source)
        self.assertIn("def font_probe_report_path", source)
        self.assertIn("def write_font_probe_report_cache", source)
        self.assertIn("--- 字体探针报告 ---", source)
        self.assertIn('self.track_usage_event("font_rule_learned" if ok else "font_rule_failed"', source)
        self.assertIn("self.learn_font_probe_rule(font_name, candidates, result)", source)
        self.assertIn("fc-list", source)
        self.assertIn("def font_name_variants", source)
        self.assertIn("self.font_file_aliases", source)
        self.assertIn("def register_font_candidate", bridge_source)
        self.assertIn("def font_candidate_probe", bridge_source)
        self.assertIn("accepted_candidate=accepted_candidate", bridge_source)
        self.assertIn("candidate_attempts=candidate_attempts", bridge_source)
        self.assertIn("manager.AddFont(path, name)", bridge_source)
    ...
    def test_toolbox_pages_do_not_show_full_panel_button_or_auto_compact(self) -> None:
        source = (PYSIDE_DIR / "app.py").read_text(encoding="utf-8")
        self.assertIn('QLabel(APP_NAME)', source)
        self.assertIn('self.side_tabs.addTab(self.detection_tab, "黑帧夹帧检测")', source)
        self.assertIn('self.side_tabs.addTab(self.text_tab, "字幕")', source)
        self.assertNotIn("layout.addWidget(self.compact_restore_btn", source)
        animate_body = source.split("def animate_current_tab", 1)[1].split("def install_button_motion", 1)[0]
        self.assertNotIn("set_text_compact_mode(True)", animate_body)
        self.assertNotIn("set_font_compact_mode(True)", animate_body)

    def test_timeline_row_keeps_refresh_and_io_visible(self) -> None:
        source = (PYSIDE_DIR / "app.py").read_text(encoding="utf-8")
        self.assertIn("target_row = QHBoxLayout()", source)
        self.assertIn("io_row = QHBoxLayout()", source)
        self.assertIn("self.timeline_combo.setMaximumWidth(430)", source)
        ...
        self.assertIn("self.read_marks_btn.setFixedWidth(112)", source)

    def test_text_and_font_tabs_stay_as_full_toolbox_pages(self) -> None:
        source = (PYSIDE_DIR / "app.py").read_text(encoding="utf-8")
        self.assertNotIn('QPushButton("文字小窗")', source)
        self.assertIn('self.side_tabs.addTab(self.text_tab, "字幕")', source)
        self.assertIn('self.side_tabs.addTab(self.font_tab, "字体")', source)
        animate_body = source.split("def animate_current_tab", 1)[1].split("def install_button_motion", 1)[0]
        self.assertNotIn("set_text_compact_mode(True)", animate_body)
        self.assertNotIn("set_font_compact_mode(True)", animate_body)
    ...
    def test_timeline_group_has_full_timeline_button(self) -> None:
        ...
        self.assertIn('QPushButton("读取入出点")', source)
        self.assertIn("target_row.addWidget(refresh)", source)
        self.assertIn("target_row.addWidget(self.read_marks_btn)", source)
        self.assertIn("io_row.addWidget(self.full_timeline_btn)", source)
        self.assertIn("self.io_in.setMaximumWidth(154)", source)
    ...
    def test_theme_modes_and_startup_foreground_activation_exist(self) -> None:
        ...
        self.assertIn("def enforce_resolve_window_level", source)
        self.assertIn("self.window_level_timer.setInterval(1000)", source)
        self.assertIn("self.window_level_timer.timeout.connect(self.enforce_resolve_window_level)", source)
        self.assertIn("macos_resolve_is_frontmost()", source)
        self.assertIn("self.isMinimized() or not self.isVisible()", source)
        self.assertIn("apply_macos_window_level(self, False)", source)
        self.assertIn("NSFloatingWindowLevel if floating else NSNormalWindowLevel", source)
        self.assertIn("def stat_card_style", source)
        self.assertIn("self.refresh_result_card_styles()", source)
        self.assertIn('"theme": self.theme_name', source)
        self.assertIn('self.apply_theme(str(data.get("theme") or "default"), update_combo=True, persist=False)', source)
        self.assertIn('self.apply_theme("default", update_combo=True, persist=False)', source)
    ...
    def test_font_panel_removes_batch_replace_button_and_text_table_allows_edit_undo(self) -> None:
        ...
        self.assertIn("if restored_delete:", source)
        self.assertNotIn("self.text_undo_btn.setEnabled(bool(self.text_undo_stack))\n        self.scan_text_layers()", source)
        bridge_source = (PYSIDE_DIR / "resolve_bridge.py").read_text(encoding="utf-8")
        self.assertIn('if str(ITEM.get("track_type", "")) == "subtitle":', bridge_source)
        self.assertLess(
            bridge_source.index('if str(ITEM.get("track_type", "")) == "subtitle":'),
            bridge_source.index("set_item_text(target"),
        )
    ...
    def test_font_style_library_and_16x9_preview_exist(self) -> None:
        ...
        self.assertIn('QPushButton("只复制字体")', source)
        self.assertIn("def copy_selected_textplus_font_only", source)
        self.assertIn('self.font_style_clipboard = {"Font": family}', source)
        self.assertIn("应用样式时只会改 Font/Style", source)
        ...
        self.assertIn("browser_panel.setMaximumWidth(560)", source)
        self.assertIn("right_panel.setMinimumWidth(620)", source)
        self.assertIn('self.font_table.setHorizontalHeaderLabels(["#", "Timecode", "文字 / 说明", "层", "当前字体", "状态"])', source)
        self.assertIn("self.font_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.Stretch)", source)
    ...
    def test_text_and_detection_layout_keep_primary_content_readable(self) -> None:
        source = (PYSIDE_DIR / "app.py").read_text(encoding="utf-8")

        self.assertIn("self.text_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.Stretch)", source)
        self.assertNotIn("self.text_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.Interactive)", source)
        self.assertIn("self.timeline_combo.setMaximumWidth(430)", source)
        self.assertIn("self.timeline_combo.setMinimumContentsLength(22)", source)
        self.assertIn("self.io_in.setMaximumWidth(154)", source)
        self.assertIn("self.io_out.setMaximumWidth(154)", source)
        self.assertIn("label.setFixedWidth(106)", source)
        self.assertIn("label.setAlignment(Qt.AlignRight | Qt.AlignVCenter)", source)
        self.assertIn("layout.setColumnStretch(0, 0)", source)
        self.assertIn("layout.setColumnStretch(1, 0)", source)
        self.assertIn("slider.setMaximumWidth(180)", source)
    ...
    def test_srt_to_textplus_uses_drb_template_and_audio_mark_marks_clips_only(self) -> None:
        ...
        self.assertIn('QPushButton("识别选中BPM")', app_source)
        self.assertIn('QPushButton("生成节拍标记")', app_source)
        self.assertIn('QPushButton("清除节拍标记")', app_source)
        self.assertIn('QPushButton("媒体池探针")', app_source)
        self.assertIn("self.media_pool_probe_btn.setVisible(False)", app_source)
        self.assertNotIn("actions.addWidget(self.media_pool_probe_btn)", app_source)
        self.assertIn("def estimate_selected_audio_bpm", app_source)
        self.assertIn("def mark_selected_audio_beats", app_source)
        self.assertIn("def run_media_pool_probe", app_source)
        self.assertIn("def render_audio_bpm", app_source)
        self.assertIn("def add_bpm_grid_markers", bridge_source)
        self.assertIn("def clear_bpm_markers", bridge_source)
        self.assertIn("def probe_media_pool_api", bridge_source)
        self.assertIn('requested_key and not requested_key.startswith("group:textplus:track:")', bridge_source)
        self.assertIn('action in {"convert_srt_textplus", "apply_style"}', bridge_source)
        self.assertIn("[QH-BPM]", bridge_source)
        self.assertIn("MediaPool / Folder / MediaPoolItem / TimelineItem diagnostic probe", bridge_source)
        self.assertIn("--- 媒体池/API 探针 ---", app_source)
        ...
        self.assertIn("GetSelectedItems", bridge_source)
        self.assertIn("playhead_audio_clip", bridge_source)
        self.assertIn("needs_selection", bridge_source)
        self.assertIn("播放头下有", bridge_source)
        self.assertIn("QInputDialog.getItem", app_source)
        self.assertIn("SRT2TEXT_GROUP_THRESHOLD = 40", bridge_source)
        self.assertIn("dominant_font_ratio >= 0.75", bridge_source)
        self.assertIn("dominant_center_ratio >= 0.60", bridge_source)
        self.assertIn("快速折叠显示", bridge_source)
        ...
        self.assertIn('"endFrame": max(0, source_dur - 1)', bridge_source)
        self.assertIn('safe(lambda item=media_item: item.GetName(), "")', bridge_source)
        self.assertIn('"convert_srt_textplus"', bridge_source)
        self.assertIn("tool.GetInputList()", bridge_source)
        self.assertIn("obj.GetAttrs()", bridge_source)
        self.assertIn('"Opacity1"', bridge_source)
        self.assertIn('"Position"', bridge_source)
        self.assertIn("should_skip_textplus_style_key", bridge_source)
        self.assertIn("self.caption_template_combo = QComboBox()", app_source)
        self.assertIn("self.srt2text_marker_checkbox = QCheckBox", app_source)
        self.assertIn("self.srt2text_marker_checkbox.setChecked(False)", app_source)
        self.assertIn("写入转换标记", app_source)
        ...
        self.assertIn("write_markers = bool", app_source)
        self.assertIn("WRITE_SRT2TEXT_MARKERS = bool", bridge_source)
        self.assertIn("def is_textplus_template_item", bridge_source)
        self.assertIn("is_subtitle_or_audio_media_item", bridge_source)
        self.assertIn("媒体池选中候选", bridge_source)
        self.assertIn("kind_text = str(template.get", app_source)
        self.assertIn('for key in ("Clip Name", "File Name", "Name")', bridge_source)
        self.assertIn('"Fusion标题"', bridge_source)
        self.assertIn('"timeline", "时间线"', bridge_source)
        self.assertIn("return False", bridge_source)
        self.assertIn('"textplus": True', bridge_source)
        self.assertIn('"markers_written": markers_written', bridge_source)
        self.assertIn("多数字幕正文为空或是 Resolve 内部名", bridge_source)
        self.assertIn("未执行降级插入", bridge_source)
