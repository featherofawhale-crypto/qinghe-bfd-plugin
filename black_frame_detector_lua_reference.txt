...
function Main()
    ...
    local function check_progress_panel(stage_name)
    ...
    if params.merge_mode then
        ...
        for file_path, file_clips in pairs(ffmpeg_file_dedup) do
            ...
            check_progress_panel(string.format("FFmpeg %d/%d", file_count, #ffmpeg_unique_files))
    ...
    if params.detect_duplicate ~= false and config.DUPLICATE.ENABLED then
        ...
        -- 8b: 基于帧指纹的内容重复检测
        dlog(string.format("阶段8b入口: CONTENT_DETECT_ENABLED=%s complex_render=%s",
            tostring(config.DUPLICATE.CONTENT_DETECT_ENABLED), tostring(complex_render_done)))
        if config.DUPLICATE.CONTENT_DETECT_ENABLED then
            ...
            dlog("阶段8b: CONTENT_DETECT_ENABLED=false，跳过")
        end
    else
