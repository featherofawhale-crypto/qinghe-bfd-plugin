...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.srt_poll_timer = QTimer(self)
        self.srt_poll_timer.setInterval(5000)
        self.srt_poll_timer.timeout.connect(self.poll_srt_refresh)
    ...
    def _build_text_tab(self) -> QWidget:
        ...
        # --- SRT 直接编辑区 ---
        srt_sep = QFrame()
        srt_sep.setFrameShape(QFrame.HLine)
        srt_sep.setStyleSheet("background:#d9e1ea;max-height:1px;")
        layout.addWidget(srt_sep)

        srt_header = QHBoxLayout()
        srt_label = QLabel("SRT 编辑")
        srt_label.setStyleSheet("font-weight:700;font-size:12px;")
        self.srt_count_label = QLabel("")
        self.srt_count_label.setObjectName("Muted")
        srt_header.addWidget(srt_label)
        srt_header.addWidget(self.srt_count_label)
        srt_header.addStretch()
        srt_refresh_btn = QPushButton("刷新")
        srt_refresh_btn.setMaximumWidth(60)
        srt_refresh_btn.clicked.connect(self.manual_refresh_srt)
        set_tip(srt_refresh_btn, "从时间线读取最新字幕(或等5秒自动刷新)")
        srt_header.addWidget(srt_refresh_btn)
        layout.addLayout(srt_header)

        self.srt_editor = QPlainTextEdit()
        self.srt_editor.setMinimumHeight(150)
        self.srt_editor.setMaximumHeight(250)
        self.srt_editor.setPlaceholderText("切换到文字标签页自动加载时间线字幕...")
        self.srt_editor.textChanged.connect(self._on_srt_editor_changed)
        layout.addWidget(self.srt_editor)

        srt_btn_row = QHBoxLayout()
        self.srt_apply_btn = QPushButton("应用 SRT")
        self.srt_apply_btn.setEnabled(False)
        self.srt_apply_btn.clicked.connect(self.apply_srt_to_timeline)
        set_tip(self.srt_apply_btn, "应用SRT到时间线（全量替换，保留时间码）")
        srt_btn_row.addWidget(self.srt_apply_btn)
        self.srt_restore_btn = QPushButton("恢复")
        self.srt_restore_btn.setVisible(False)
        self.srt_restore_btn.clicked.connect(self.restore_srt_backup)
        set_tip(self.srt_restore_btn, "恢复到应用前的字幕状态")
        srt_btn_row.addWidget(self.srt_restore_btn)
        self.srt_status = QLabel("")
        self.srt_status.setObjectName("Muted")
        srt_btn_row.addWidget(self.srt_status)
        srt_btn_row.addStretch()
        layout.addLayout(srt_btn_row)
    ...
    def manual_refresh_srt(self) -> None:
        self._srt_text_hash = ""
        self.poll_srt_refresh()

    def _on_srt_editor_changed(self) -> None:
        if self._srt_loading:
            return
        current = self.srt_editor.toPlainText()
        self.srt_apply_btn.setEnabled(len(current.strip()) > 0 and current != self._srt_text_hash)

    def poll_srt_refresh(self) -> None:
        if self._srt_loading or self.worker is not None:
            return
        self._srt_loading = True
        try:
            selected = self.timeline_combo.currentData() or {"index": 1}
            tl_index = int(selected.get("index", 1))
            result = self.bridge.export_subtitles_srt(tl_index)
            if result.get("ok"):
                new_srt = result.get("srt", "")
                if new_srt != self._srt_text_hash:
                    cursor = self.srt_editor.textCursor()
                    scroll_val = self.srt_editor.verticalScrollBar().value()
                    self.srt_editor.setPlainText(new_srt)
                    self.srt_editor.verticalScrollBar().setValue(scroll_val)
                    self._srt_text_hash = new_srt
                    self.srt_count_label.setText("(%d 条)" % result.get("count", 0))
                    self.srt_apply_btn.setEnabled(False)
        finally:
            self._srt_loading = False

    def apply_srt_to_timeline(self) -> None:
        srt_content = self.srt_editor.toPlainText()
        if not srt_content.strip():
            self.srt_status.setText("SRT内容为空。")
            return
        selected = self.timeline_combo.currentData() or {"index": 1}
        tl_index = int(selected.get("index", 1))

        # Save backup before applying
        original_srt = getattr(self, '_srt_text_hash', '')
        backup_srt = original_srt if original_srt else srt_content
        self._srt_backup = backup_srt

        self.srt_status.setText("正在应用...")
        QApplication.processEvents()
        result = self.bridge.replace_subtitles_from_srt(tl_index, srt_content, original_srt)
        self.srt_status.setText(str(result.get("message", "")))
        if result.get("ok"):
            self._srt_text_hash = srt_content
            self.srt_apply_btn.setEnabled(False)
            self.srt_restore_btn.setVisible(True)
        else:
            self.srt_restore_btn.setVisible(True)
            self.srt_status.setText(self.srt_status.text() + " (可按恢复按钮回退)")
        self._log("SRT: " + str(result.get("message", "")))

    def restore_srt_backup(self) -> None:
        backup = getattr(self, '_srt_backup', '')
        if not backup:
            self.srt_status.setText("没有可恢复的备份。")
            return
        selected = self.timeline_combo.currentData() or {"index": 1}
        tl_index = int(selected.get("index", 1))
        self.srt_status.setText("正在恢复备份...")
        QApplication.processEvents()
        result = self.bridge.replace_subtitles_from_srt(tl_index, backup, "")
        if result.get("ok"):
            self.srt_editor.setPlainText(backup)
            self._srt_text_hash = backup
            self.srt_apply_btn.setEnabled(False)
            self.srt_restore_btn.setVisible(False)
        self.srt_status.setText("已恢复: " + str(result.get("message", "")))
        self._log("SRT恢复: " + str(result.get("message", "")))
    ...
    def animate_current_tab(self) -> None:
        ...
        # SRT轮询：仅在文字标签页可见时启用
        if widget == self.text_tab:
            if not self.srt_poll_timer.isActive():
                self.srt_poll_timer.start()
                self.poll_srt_refresh()
        else:
            if self.srt_poll_timer.isActive():
                self.srt_poll_timer.stop()
    ...
    def poll_timeline_change(self) -> None:
        ...
        if current_uid and self._last_timeline_uid and current_uid != self._last_timeline_uid:
            self._log(f"检测到时间线切换，自动刷新...")
            self.refresh_timelines()
            self._capture_current_timeline_uid()
            self._log("时间线列表已更新。")
